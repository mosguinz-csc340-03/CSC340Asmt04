...
void B1() {
    ...
    std::cout << "Created an `std::string` using `new`; the pointer is captured to a variable `a`" << std::endl
              << "Dereferencing `a` yields: " << *a << std::endl << std::endl;

    std::string *b = a;
    std::cout << "Created another pointer `b` to have the same value as `a`" << std::endl
              << "Both pointers `a` and `b` points to " << a << std::endl << std::endl;
    ...
    std::cout << "This means that `a` and `b` are now dangling because they both point to deallocated memory"
              << std::endl
              << "Attempting to apply `delete` on to `b` will result in an error, for example:" << std::endl
              << std::endl;

    // delete b;

    // In Clang,
    std::cout << "PartB_Experiments(36739," << a << ") malloc: *** error for object " << a
              << ": pointer being freed was not allocated" << std::endl
              << "PartB_Experiments(36739," << a << ") malloc: *** set a breakpoint in malloc_error_break to debug"
              << std::endl;
